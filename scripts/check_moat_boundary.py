...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awm
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awm-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awm/__init__.py", "awm/cli.py", "awm/predict.py", "awm/scope.py", "awm/store.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awm-0.0.0-py3-none-any.whl"
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awm-0.0.0.tar.gz"
